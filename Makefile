CC=gcc
CXX=g++
CFLAGS=-D_GNU_SOURCE -I./kv_store -I./test_director -Wno-write-strings -g
...
%.o: %.c
	$(CC) -c -o $@ $< $(CFLAGS) -I./opmock/opmock2/support/ -include stdbool.h -I./test/mocks

%.o: %.cpp
	$(CXX) -c -o $@ $< $(CFLAGS) -I./cpputest/include -I./opmock/opmock2/support/ -I./test/mocks

test/mocks/%_stub.c: kv_store/%.h
	./opmock/bin/linux/opmock2 -i $< -o test/mocks -I/usr/include -I/usr/include/x86_64-linux-gnu -I/usr/lib/gcc/x86_64-linux-gnu/4.7/include -I/usr/lib/gcc/x86_64-linux-gnu/4.7/include-fixed
	sed -i "s/bool/int/g" $@
...
kv_store_unittest: opmock/opmock2/support/opmock.o test/mocks/kv_append_point_stub.o libcpputest.a test/kv_store_unittest.o kv_store/kv_store.o test/kv_block_allocator_mock.o test/kv_directory_mock.o test/kv_block_mock.o test/kv_block_array_mock.o test/kv_store_replay_mock.o
	$(CXX) -o $@ $^ $(CFLAGS) -L. -lcpputest
	./kv_store_unittest
...
clean:
	rm $(CPPU_TEST_OBJS) $(KV_STOR_OBJS) $(TEST_DIRECTOR_OBJS) test/*.o *.o kv_harness kv_stress *_unittest libcpputest.a test/mocks/* opmock/opmock2/support/opmock.o
