...
int opmock_call_stack_push;
int opmock_call_stack_pop;
...
void opmock_test_reset()
{
    ...
    opmock_call_stack_push = 0;
    opmock_call_stack_pop = 0;
	...
	for(counter = 0; counter < opmock_test_counter; counter++) {
		...
		if(opmock_test_error > previous_error) {
			...
			if(report != NULL) {
				...
				char messages [10000U];
				opmock_sprint_error_messages(messages, 10000U);
...
int cmp_bool(void *a, void *b, const char * name, char * message){
  _Bool my_a = *((_Bool *)a);
  _Bool my_b = *((_Bool *)b);
  if(my_a == my_b) {
    return 0;
  ...
  snprintf(message, OP_MATCHER_MESSAGE_LENGTH,
           ...
           name, my_b ? "true" : "false", my_a ? "true" : "false");
  return -1;
}
...
int opmock_get_next_call_stack_push_pos()
{
    int next_pos = opmock_call_stack_push + 1;
    if (next_pos == OP_MAX_ERROR_MESSAGE) {
        return 0;
    }
    else {
        return next_pos;
...
void opmock_increment_stack_push_pos() {
    opmock_call_stack_push = opmock_get_next_call_stack_push_pos();
}

int opmock_is_call_stack_full()
{
    return opmock_get_next_call_stack_push_pos() == opmock_call_stack_pop;
}

void opmock_add_call(char * operation)
{
	if(!opmock_is_call_stack_full())
	{
		snprintf(opmock_call_stack_array[opmock_call_stack_push].message, OP_ERROR_MESSAGE_LENGTH, "%s", operation);
        opmock_increment_stack_push_pos();
	}
	else
	{
		snprintf(opmock_call_stack_array[opmock_call_stack_push].message,
            OP_ERROR_MESSAGE_LENGTH, "Too many calls (%d), can't store all of them.", OP_MAX_ERROR_MESSAGE);
	}
}

char * opmock_get_current_call()
{
	return opmock_call_stack_array[opmock_call_stack_pop].message;
}

int opmock_is_call_stack_empty()
{
    return opmock_call_stack_push == opmock_call_stack_pop;
}

int opmock_get_next_call_stack_pop_pos()
{
    int next_pos = opmock_call_stack_pop + 1;
    if (next_pos == OP_MAX_ERROR_MESSAGE) {
        return 0;
    }
    else {
...
void opmock_increment_stack_pop_pos() {
    opmock_call_stack_pop = opmock_get_next_call_stack_pop_pos();
}

void opmock_pop_call()
{
	if(!opmock_is_call_stack_empty()) {
		opmock_increment_stack_pop_pos();
	}
}
...
void opmock_sprint_error_messages(char *messages, size_t max_size)
{
	int i;
	int max = opmock_error_message < OP_MAX_ERROR_MESSAGE ? opmock_error_message : OP_MAX_ERROR_MESSAGE;
	strcpy(messages, "");
	...
	for(i = 0; i < max; i++)
	{
		size_t len = strlen(opmock_error_message_array[i].message)+1;/* +1 for the \n (accumulated) */
		sum += len;
		if(max_size >= (sum+1)) { /* +1 for the NULL termination (not accumulated) */
...
void opmock_remove_order_errors()
{
    int err_msg_read;
    int err_msg_write = 0;
    for (err_msg_read = 0; err_msg_read < opmock_error_message; err_msg_read++) {
        if (strstr(opmock_error_message_array[err_msg_read].message, "WARNING : got call to") != NULL)
        {
            // do not copy from this read index
            // do not advance write index
        }
        else
        {
            if (err_msg_read != err_msg_write)
            {
                // need to shift down
                opmock_error_message_array[err_msg_write] = opmock_error_message_array[err_msg_read];
            }
            err_msg_write++;
        }
    }
    opmock_error_message = err_msg_write;
}
