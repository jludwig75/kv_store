...
void writeFunctionIgnoreAndReturnBody ( OpFunction *one_func, OpRecord *one_rec, std::ofstream &out );
void writeFunctionIgnoreIfNotExpectedBody ( OpFunction *one_func, OpRecord *one_rec, std::ofstream &out );
...
void writeIgnoreOtherCallsMacro( const std::string &filename, std::ofstream &out );
...
llvm::cl::list<std::string> clWarnings("W", llvm::cl::desc("[Optional] Passed to clang (warning options). May be used multiple times."), llvm::cl::value_desc("warning option"), llvm::cl::ZeroOrMore, llvm::cl::Prefix);
llvm::cl::list<std::string> clFeatures("f", llvm::cl::desc("[Optional] Passed to clang (feature options). May be used multiple times."), llvm::cl::value_desc("feature option"), llvm::cl::ZeroOrMore, llvm::cl::Prefix);
...
int main ( int argc, char **argv )
{
    ...
    for (unsigned i = 0; i != clWarnings.size(); ++i)
    {
        std::string newString = "-W" + clWarnings[i];
        newWarnings.push_back(newString);
    }

    std::vector<std::string> newFeatures;
    for (unsigned i = 0; i != clFeatures.size(); ++i)
    {
        std::string newString = "-f" + clFeatures[i];
        newFeatures.push_back(newString);
    }
    ...
    // append the warnings
    for (unsigned i = 0; i != newWarnings.size(); ++i)
    {
        clangParamsList.push_back(newWarnings[i].c_str());
    }

    // append the features
    for (unsigned i = 0; i != newFeatures.size(); ++i)
    {
        clangParamsList.push_back(newFeatures[i].c_str());
    }
    ...
    clang::FileManager *FileMgr = new clang::FileManager( fso );
    ...
    AST = clang::ASTUnit::LoadFromCompilerInvocation ( CI, std::make_shared< clang::PCHContainerOperations >(), DiagsRef, FileMgr );
                      ...
                      std::string prefix, bool useQuotes )
{
    ...
    // @ddh hack: MSVC doesn't support C99 snprintf, so fall back to _snprintf.
    outc << "#if defined(_MSC_VER) && _MSC_VER < 1900\n";
    outc << "#define snprintf _snprintf\n";
    outc << "#define vsnprintf _vsnprintf\n";
    outc << "#endif\n\n";

    std::string bodies_string;
    std::string reset_all_body;
    std::string verify_all_body;
    std::string ignore_other_body;
    ...
    ignore_other_body = "void " + removeExt ( filename ) + "_stub_ignore_other_calls()\n{\n";
    ignore_other_body += "    opmock_add_reset_callback(opmock_reset_all_mocks_in_this_header);\n";
    ignore_other_body += "    opmock_add_verify_callback(opmock_verify_all_mocks_in_this_header);\n\n";
            ...
            it != functionList.end(); ++it )
    {
        ...
        {
            std::cout << "Note: " << fname << " is inlined, skipped\n";
        }
        else
        {
            nb_func++;
            ...
            for ( unsigned i = 0; i < nbParam; i++ )
            {
                ...
                assignList += "    " + fname + "_struct_inst.calls["
                              ...
                              + name + " = ";
// @ddh hack: This causes MSVC fits.
                if ( isArray || isPointer )
                {
                    assignList += "(void *)";
                }
            ...
            // function to ignore mocks without expectations in this header
            ignore_other_body += "    " + fname + "_IgnoreIfNotExpected();\n";
...
// @ddh hack: This causes MSVC fits.
            ...
            bodies_string += "    if (" + fname + "_struct_inst.isInIgnoreMode)\n    {\n";
            if ( !isVoid )
            {
            ...
            bodies_string += "    " + fname + "_struct_inst.isInIgnoreMode = false;\n";
...
// @ddh hack: This causes MSVC fits.
            ...
            // mock ignore and return implementation
            bodies_string += "void " + fname + "_IgnoreAndReturn (";
            if ( !isVoid )
            {
                bodies_string += retPrefix + " to_return";
                if ( retSuffix.size() > 0 )
            ...
            bodies_string += "    " + fname + "_struct_inst.expectedCalls = 0;\n";
            bodies_string += "    " + fname + "_struct_inst.actualCalls = 0;\n";
            bodies_string += "    " + fname + "_struct_inst.isInIgnoreMode = true;\n";
...
// @ddh hack: This causes MSVC fits.
            ...
            // mock ignore if not expected implementation
            bodies_string += "static void " + fname + "_IgnoreIfNotExpected ()\n{\n";
            bodies_string += "    if(!" + fname + "_struct_inst.isInIgnoreMode &&\n";
            bodies_string += "        (" + fname + "_struct_inst.callback == NULL) &&\n";
            bodies_string += "        (" + fname + "_struct_inst.expectedCalls == 0))\n";
            bodies_string += "    {\n";
            ...
            if ( !isVoid )
            {
                ...
                bodies_string += "        " + retPrefixNoConst + " to_return";
                if ( canonRetSuffix.size() > 0 )
                {
                    bodies_string += " " + canonRetSuffix;
                }
                bodies_string += ";\n\n";

                bodies_string += "        memset(&to_return, 0, sizeof(to_return));\n";
                bodies_string += "        " + fname + "_struct_inst.calls[0].to_return = to_return;\n";
            }
            bodies_string += "        " + fname + "_struct_inst.isInIgnoreMode = true;\n";
            bodies_string += "    }\n";
            bodies_string += "}\n\n";
            ...
            // output ignore and return function in the header
            hout << "void" << " " << fname << "_IgnoreAndReturn (";
    ...
    writeIgnoreOtherCallsMacro( filename, hout );
                        ...
                        std::string &funcToSkip, std::string &funcToKeep )
{
            ...
            it != functionList.end(); ++it )
    {
        ...
        if ( new_func )
        {
            ...
            {
                std::cout << "Filtering function " << new_func->qualifiedName << " as it is variadic\n";
                delete new_func;
            }
            else if(new_func->isInlined)
            {
                std::cout << "Filtering function " << new_func->qualifiedName << " as it is inlined\n";
                delete new_func;
            }
            else
            {
            ...
            it != opFunctions.end(); ++it )
    {
        ...
        if ( ( one_func->isTemplate == false ) && ( one_func->isOperatorOverload == false ) )
        {
            ...
            // function IgnoreAndReturn
            hout << ns_left << "void" << " " << one_func->name << "_IgnoreAndReturn (";
            // return value
            if ( !one_func->returnsVoid )
            {
                hout << one_func->retSignatureLeft << " to_return";
        ...
        if ( !one_rec->isTemplate )
        {
                    ...
                    itm != one_rec->methods.end(); ++itm )
            {
                        ...
                        ( one_func->isTemplate == false ) )
                {
                    ...
                    // function IgnoreAndReturn
                    hout << "    static void" << " " << one_func->name << "_IgnoreAndReturn (";
                    // return value
                    if (!one_func->returnsVoid)
                    {
                        hout << one_func->retSignatureLeft << " to_return";
                    ...
                    // function IgnoreIfNotExpected
                    hout << "    static void" << " " << one_func->name << "_IgnoreIfNotExpected (";
                    hout << ");\n";
                }
            }
            hout << "};\n";
    ...
    writeIgnoreOtherCallsMacro( SplitFilename ( input_file ), hout );
            ...
            it != opFunctions.end(); ++it )
    {
                ...
                ( one_func->isTemplate == false ) )
        {
            ...
            writeFunctionIgnoreAndReturnBody ( one_func, NULL, outc );
            writeFunctionIgnoreIfNotExpectedBody ( one_func, NULL, outc );
        }
    }
        ...
        if ( !one_rec->isTemplate )
        {
                    ...
                    it2 != one_rec->methods.end(); ++it2 )
            {
                        ...
                        ( one_func->isTemplate == false ) )
                {
                    ...
                    writeFunctionIgnoreAndReturnBody ( one_func, one_rec, outc );
                    writeFunctionIgnoreIfNotExpectedBody ( one_func, one_rec, outc );
                }
    ...
    // function to ignore mocks without expections in this header
    if ( nb_func > 0 )
    {
        outc << "void " << removeExt ( filename ) << "_stub_ignore_other_calls()\n{\n";
        outc << "    opmock_add_reset_callback(opmock_reset_all_mocks_in_this_header);\n";
        outc << "    opmock_add_verify_callback(opmock_verify_all_mocks_in_this_header);\n\n";
        // first C style free functions
        for ( std::vector<OpFunction *>::iterator it = opFunctions.begin();
                it != opFunctions.end(); ++it )
        {
            ...
            if ( !one_rec->isTemplate )
            {
                    ...
                    itm != one_rec->methods.end(); ++itm)
                {
                        ...
                        (one_func->isTemplate == false))
                    {
                        ...
                        classPrefix += one_rec->name;
                        classPrefix += "_Mock::" + one_func->name + "_IgnoreIfNotExpected(); \n";
                        outc << classPrefix;
                    }
                }
            }
                              ...
                              std::map< std::string, NameMap * > &nsMap )
{
    ...
    if ( fdecl->hasBody() )
    {
        // @ddh hack: Don't warn about inlined functions.
        // std::cout << "WARNING " << new_func->qualifiedName << " is inlined\n";
        new_func->isInlined = true;
    }

    clang::CXXConstructorDecl *c_decl =
...
void writeFunctionBody ( OpFunction *one_func, OpRecord *one_rec, std::ofstream &out )
{
    ...
    out << "    if (" << struct_inst << ".isInIgnoreMode)\n    {\n";
    out << "        return";
    if ( one_func->returnsVoid )
    {
        out << ";\n";
...
void writeFunctionMockResetBody ( OpFunction *one_func, OpRecord *one_rec, std::ofstream &out )
{
    ...
    out << structName << ".isInIgnoreMode = false;\n";
...
void writeFunctionIgnoreAndReturnBody ( OpFunction *one_func, OpRecord *one_rec, std::ofstream &out )
{
    ...
    out << "    opmock_add_reset_callback(opmock_reset_all_mocks_in_this_header);\n";
    out << "    opmock_add_verify_callback(opmock_verify_all_mocks_in_this_header);\n";
    out << "    " << structi << ".callback = NULL;\n";
    if ( one_rec && ( !one_func->isStatic ) )
    {
        out << "    " << structi << ".callbackInst = NULL;\n";
    }

    out << "    " << structi << ".expectedCalls = 0;\n";
    out << "    " << structi << ".actualCalls = 0;\n";
    out << "    " << structi << ".isInIgnoreMode = true;\n";
...
void writeFunctionIgnoreIfNotExpectedBody ( OpFunction *one_func, OpRecord *one_rec, std::ofstream &out )
{
    ...
    out << "_IgnoreIfNotExpected ()\n{\n";

    out << "    if(!" << structi << ".isInIgnoreMode &&\n";
    out << "        (" << structi << ".callback == NULL) &&\n";
    if ( one_rec && ( !one_func->isStatic ) )
    {
        out << "        (" << structi << ".callbackInst == NULL) &&\n";
    ...
    {
        out << "        " << one_func->retStructType << " to_return;\n\n";
        out << "        memset(&to_return, 0, sizeof(to_return));\n";
        out << "        " << structi << ".calls[0].to_return = to_return;\n";
    }
    out << "        " << structi << ".isInIgnoreMode = true;\n";
    out << "    }\n";
...
void writeIgnoreOtherCallsMacro( const std::string &filename, std::ofstream &out )
{
    // Write a function chain_foo_stub_ignore_other_calls() that will call
    // foo_stub_ignore_other_calls() for each mocked header, putting all
    // mocks without any expectations into "ignore" mode.

    // We create this function by constructing a static void function
    // chain_foo_stub_ignore_other_calls that calls both
    // foo_stub_ignore_other_calls and the previous
    // chain_prevfoo_stub_ignore_other_calls.
    // We then define OPMOCK_IGNORE_OTHER_CALLS to call the chain_* function
    // we just created. By the time all mocked headers have been included,
    // OPMOCK_IGNORE_OTHER_CALLS will be defined to the latest which cains to
    // the previous, etc.
    
    std::string func = removeExt ( filename ) + "_stub_ignore_other_calls()";
    out << "void " + func + ";\n\n";

    // Define an empty macro to be used for the first iteration
    out << "#ifndef OPMOCK_IGNORE_OTHER_CALLS\n";
    out << "#define OPMOCK_IGNORE_OTHER_CALLS()\n";
    out << "#endif\n\n";

    // Create the static chain funcion for this level and call the previous chain function.
    out << "inline static void chain_" + func + " {\n";
    out << "    " + func + ";\n";
    out << "    OPMOCK_IGNORE_OTHER_CALLS();\n";
    out << "}\n";

    // Replace the old chain function with the new one
    out << "#undef OPMOCK_IGNORE_OTHER_CALLS\n";
    out << "#define OPMOCK_IGNORE_OTHER_CALLS() chain_" + func + "\n";
}
